...
public class CreateUpdateDeleteTest extends BaseTest{
    ...
    public void tearDown(){
        given().baseUri(baseUrl).basePath("api/books/id"+book.getId()).headers(headers).delete().then();
    ...
    public void deleteNonExistingBook(){
        Response response =
                given().baseUri(baseUrl).basePath("api/books/id/260").headers(headers).when().delete().then().extract().response();
    ...
    public void updateBook(){
        ...
        JsonPath json = response.jsonPath();
        var bookUpdateResponse = json.getObject("$", BookDeleteResponse.class);
